/**
 * Learn — open-ended skill distillation (hermes-agent /learn parity, no slash
 * commands). There is no engine and no model tool: `buildLearnRequest` builds
 * ONE standards-guided prompt that any surface submits as a NORMAL user turn.
 * The live agent gathers the described sources with its existing tools and
 * authors the skill via `skills.save` / `skills.writeFile`.
 *
 * The request is open-ended — a directory of code, an API doc URL, a workflow
 * the user just walked the agent through in this conversation, or pasted
 * notes. Small sources get one tight SKILL.md; large prose sources (books,
 * paper stacks, specs, doc corpora) get the knowledge-base layout — a lean
 * SKILL.md index plus per-chapter `references/` files loaded on demand via
 * `skills.view(name, path)`.
 */

/** The built-in agent that runs learn requests spawned outside a conversation. */
export const LEARN_AGENT_NAME = "learn";

/** The house-style rules for authored skills (bai framing of hermes' hardline). */
const AUTHORING_STANDARDS = `\
Follow the bai skill-authoring standards exactly. These are the same hardline
rules a maintainer enforces in review:

Frontmatter (fields of skills.save):
- skill (the name): lowercase-hyphenated, <=64 chars, no spaces — it becomes
  the directory stem (~/.config/bai/skills/<name>/SKILL.md).
...
- version: 0.1.0
- author: always the literal value \`bai\`. NEVER fill it from the host
  ...
  environment-derived name is a privacy leak the user never opted into; the
  skill names itself as bai.
- platforms: declare \`[darwin]\`, \`[linux]\`, and/or \`[win32]\` (process.platform
  values) IF the skill uses OS-bound primitives (osascript => darwin;
  apt/systemctl => linux; /proc, os.setsid, signal.SIGKILL => linux;
  fcntl/termios => POSIX). Prefer fixing it cross-platform first
  (tempfile.gettempdir(), pathlib-style paths); gate only when the dependency
  is genuinely platform-bound. Omit the field for portable skills.
...
4. "## How to Run" — the canonical invocation, framed through bai tools.
...
bai-tool framing (this is what makes it a skill, not shell docs):
- Frame running scripts as "invoke through the \`bash\` tool".
- Reference bai tools by name in backticks: \`bash\`, \`fs.read\`, \`fs.write\`,
  \`fs.edit\`, \`fs.list\`, \`fs.glob\`, \`fs.grep\`, \`web.fetch\`, \`web.search\`,
  \`task\`, \`skills.view\`, \`skills.save\`, \`skills.writeFile\`.
- Do NOT name shell utilities the agent already has wrapped: say \`fs.read\`
  not cat/head/tail, \`fs.grep\`/\`fs.glob\` not grep/rg/find/ls, \`fs.edit\` not
  sed/awk, \`web.fetch\` not curl-to-scrape, \`fs.write\` not echo>file or
  heredocs.
- Third-party CLIs (ffmpeg, gh, an SDK) are fine inside a script file, but the
  prose still frames them as "invoke through the \`bash\` tool". If the skill
...
- Larger scripts/parsers belong in a \`scripts/\` file (add via
  \`skills.writeFile\`), referenced from SKILL.md by relative path — not
  inlined for the agent to re-type every run. References go in \`references/\`,
  templates in \`templates/\`.`;

/** Rules for the expansive shape: books, paper stacks, large doc corpora, specs. */
const KNOWLEDGE_SKILL_STANDARDS = `\
...
- One file per chapter or major topic under \`references/\` (e.g.
  \`references/ch04-replication.md\`), each added with \`skills.writeFile\`.
  Distill STRUCTURE, not summary: frameworks, definitions, decision rules,
  anti-patterns, key numbers and tables, with chapter/section refs back to
  the source. Bullet-dense, roughly 100-150 lines per file.
...
- SKILL.md must tell the reader to load a chapter on demand with
  \`skills.view\` (path="references/<file>") — reference files cost nothing
...
- Fold-in, don't duplicate: if a skill for this source or topic already
  exists, extend it (\`skills.save\` with the merged body, plus
  \`skills.writeFile\` for new material) instead of creating a near-duplicate
  skill.`;

/** Untrusted-source hygiene, embedded in every learn request. */
 ...
 *
 * @param request the free-text the user gave — a description of the workflow,
 *   paths, URLs, or "what I just did". Empty defaults to distilling the
 *   current conversation.
 * @returns a complete instruction the agent runs as a normal turn.
 */
export function buildLearnRequest(request: string): string {
  ...
  return (
    ...
    "have — `fs.read`/`fs.list`/`fs.glob`/`fs.grep` for local files or " +
    "directories, `web.fetch` for URLs, the current conversation history if " +
    "they referred to something you just did, and the text they pasted " +
    "as-is. Gather a small source now. For a large source, inspect enough " +
    "to map its chapters or major topics, but do not load the whole corpus " +
    "into conversation context; process it incrementally in step 2b. " +
    ...
    "2. Save the skill with `skills.save`. First check the available " +
    "skills for one covering this source or topic. If one exists, load it " +
    "with `skills.view`, then extend its SKILL.md with `skills.save` (the " +
    "merged body) and add or update supporting files with `skills.writeFile`. " +
    "Only when no matching skill exists, create one with `skills.save` and " +
    "pick a sensible name. If the procedure needs a non-trivial script, add " +
    "it under the skill's `scripts/` with `skills.writeFile` and reference " +
    "it by relative path.\n" +
    ...
    "index plus per-chapter `references/` files added with `skills.writeFile`. " +
    "If a single SKILL.md would force you to summarize away most of the " +
    "material, that is the signal to go expansive. For this layout, create " +
    "or load the skill after inventorying the source, then read, distill, " +
    "and persist one chapter/topic at a time before reading the next; " +
    "finish by reconciling the SKILL.md index with every reference file " +
    "you wrote.\n\n" +
    `${SOURCE_HYGIENE}\n\n` +
    `${AUTHORING_STANDARDS}\n\n` +
    `${KNOWLEDGE_SKILL_STANDARDS}\n\n` +
    "When done, tell the user the skill name, a one-line summary of what it " +
    "captured, and — for a knowledge-base skill — the list of reference " +
    "files it can load on demand."
  );
}
