...
describe("tool-call loop", () => {
  ...
  test("unknown agent selections are rejected at the API; deleted agents fall back at drain", async () => {
    ...
    expect(first.tools?.map((d) => d.name)).toEqual(["bash", "fs.edit", "fs.glob", "fs.grep", "fs.list", "fs.read", "fs.write", "image.generate", "notes.read", "notes.write", "plan.read", "task", "todo"]);
  });
