...
describe("plan agent flow (end-to-end)", () => {
  ...
  test("plan → plan.exit approved → mid-run switch to build (persona + tools change)", async () => {
    ...
    expect(first.tools?.map((d) => d.name)).toEqual(["fs.glob", "fs.grep", "fs.list", "fs.read", "notes.read", "plan.exit", "plan.read", "plan.write", "question", "todo", "web.fetch", "web.search"]);
    ...
    expect(third.tools?.map((d) => d.name)).toEqual(["bash", "fs.edit", "fs.glob", "fs.grep", "fs.list", "fs.read", "fs.write", "image.generate", "notes.read", "notes.write", "plan.read", "task", "todo"]);
  ...
  test("built-in plan and chat agents exist with the right tool lists", () => {
    ...
    expect(plan?.tools).toEqual(["fs.read", "fs.list", "fs.glob", "fs.grep", "web.search", "web.fetch", "notes.read", "plan.write", "plan.read", "question", "todo", "plan.exit"]);
