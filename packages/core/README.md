...
| `src/provider/` | LLM access behind one small interface; adapters for openai v7 / @anthropic-ai/sdk / the OpenAI Responses API + an OpenAI-compatible catch-all; models.dev catalog via `@opencode-ai/models` with offline snapshot fallback, a curated overlay, and multi-account API-key + OAuth credentials (`auth.json`) with a server-side login engine (`oauth/`). Vendor types never leak past adapter files. |
| `src/mcp/`      | MCP client manager (`@modelcontextprotocol/client` v2, protocol rev 2026-07-28): file-first registry (`~/.config/bai/mcp/<name>.json\|yaml`), stdio + streamable HTTP/SSE transports, OAuth token store, `mcp/<server>/<tool>` namespaced merge, resources/prompts helpers. Server-side exposure wiring is mounted by `@bai/api`. |
