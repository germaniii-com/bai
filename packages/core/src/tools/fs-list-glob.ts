 ...
 * common build output ignored), capped with an actionable note.
 */
...
export function fsListTool(roots: FsRoots): Tool {
  return {
    ...
    origin: "builtin",
    description:
      "List files and directories under a path as an indented tree. Hidden directories and build output (node_modules, dist, …) are skipped. Use fs.glob for pattern search.",
    schema: {
      type: "object",
      properties: {
    ...
    async execute(args: unknown, ctx: ToolContext): Promise<ToolResult> {
      ...
      const lines: string[] = [];
      const truncated = walk(abs, "", 0, lines, LIST_ENTRY_CAP);
      const header = lines.length === 0 ? `(empty) ${abs}` : `${abs} (${lines.length} entries${truncated ? ", truncated" : ""})`;
      return {
        content: [header, ...lines].join("\n"),
        meta: { path: abs, entries: lines.length, truncated },
      };
    },
  };
}
    ...
    async execute(args: unknown, ctx: ToolContext): Promise<ToolResult> {
      ...
      const lines: string[] =
        files.length === 0
          ...
          : files.length >= GLOB_RESULT_CAP
            ? [...files, "", `(Truncated at ${GLOB_RESULT_CAP} matches — refine the pattern or path for more.)`]
            : files;
      return {
        content: lines.join("\n"),
        meta: { pattern, base, count: files.length },
      };
    },
  };
}
