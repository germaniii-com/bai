 ...
 * shared by the curated provider overlay (`provider/overlay.ts`, which turns
 * them into catalog entries so `ProviderRegistry.resolveCredentials` can find
 * env vars + base URLs) and the image workbench's adapter registry
 * (`workbench/media/registry.ts`).
 *
 * Living at the core root keeps it neutral: both `provider/` and `workbench/`
 * may import it without a layering cycle, and it depends on nothing but plain
 * data (no `@bai/shared` import needed).
 *
